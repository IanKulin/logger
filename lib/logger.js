...
class Logger {
  constructor(options = {}) {
    ...
    this.options = {
      ...
      time: 'short',
      callerLevel: 'warn', // minimum level to include caller information
      stream: null,
      ...options,
    ...
    if (this.options.stream) {
      this.options.stream.on('error', (err) => {
        process.stderr.write(`[@iankulin/logger] stream error: ${err.message}\n`);
      });
    }
  ...
  validateOptions(options) {
    ...
    if (options.stream !== undefined && options.stream !== null) {
      if (typeof options.stream.write !== 'function') {
        throw new Error('stream option must be a Writable stream (must have a write method)');
      }
    }
    ...
    try {
      ...
      try {
        ...
        const safeEntry = {
          ...
          jsonError: `JSON stringify failed: ${error.message}`,
        };
  ...
  log(level, message, ...args) {
    ...
    const { callerFile, callerLine } = shouldIncludeCaller
      ? this.getCallerInfo()
      : { callerFile: null, callerLine: null };
    ...
    try {
      ...
      try {
        const safeEntry = {
          ...logEntry,
          formatterError: `Formatter failed: ${error.message}`,
        };
    ...
    if (this.options.stream) {
      this.options.stream.write(formattedLog + '\n');
